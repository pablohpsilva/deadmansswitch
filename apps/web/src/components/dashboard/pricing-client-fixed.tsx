...
export function PricingClient() {
  ...
  const { data: pricing, isLoading: pricingLoading } = (
    trpc as any
  ).payments.getPricing.useQuery();

  // Fetch current subscription status
  const { data: subscription, isLoading: subscriptionLoading } = (
    trpc as any
  ).payments.getSubscription.useQuery();

  // Mutations
  const createCheckout = (
    trpc as any
  ).payments.createPremiumCheckout.useMutation();
  const createLifetimeCheckout = (
    trpc as any
  ).payments.createLifetimeCheckout.useMutation();
  const createLightningCheckout = (
    trpc as any
  ).payments.createLightningCheckout.useMutation();
  const createStablecoinCheckout = (
    trpc as any
  ).payments.createStablecoinCheckout.useMutation();
  const verifyLightningPayment = (
    trpc as any
  ).payments.verifyLightningPayment.useMutation();
  const verifyStablecoinPayment = (
    trpc as any
  ).payments.verifyStablecoinPayment.useMutation();
  const cancelSubscription = (
    trpc as any
  ).payments.cancelSubscription.useMutation();
